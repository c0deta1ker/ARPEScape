function varargout = KNavi_Eb(varargin)
% KNAVI_EB MATLAB code for KNavi_Eb.fig
%      KNAVI_EB, by itself, creates a new KNAVI_EB or raises the existing
%      singleton*.
%      H = KNAVI_EB returns the handle to a new KNAVI_EB or the handle to
%      the existing singleton*.
%      KNAVI_EB('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KNAVI_EB.M with the given input arguments.
%      KNAVI_EB('Property','Value',...) creates a new KNAVI_EB or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before KNavi_Eb_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to KNavi_Eb_OpeningFcn via varargin.
%      *See GUI Options on GUIDE's Tools menu.  Choose "GUI allows only one
%      instance to run (singleton)".
% See also: GUIDE, GUIDATA, GUIHANDLES
% Edit the above text to modify the response to help KNavi_Eb
% Last Modified by GUIDE v2.5 07-Dec-2020 23:31:04
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @KNavi_Eb_OpeningFcn, ...
                   'gui_OutputFcn',  @KNavi_Eb_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before KNavi_Eb is made visible.
function KNavi_Eb_OpeningFcn(hObject,~,handles,varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to KNavi_Eb (see VARARGIN)
% Choose default command line output for KNavi_Eb
...
% UIWAIT makes KNavi_Eb wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = KNavi_Eb_OutputFcn(~,~,handles)
