...
global Energy Angle ScanFull Scan SWin DataFull DataXC Data Note RawTable HV thtM tltM aDef ScanType ACorr ECorr ...
...
aDef=[];
try
   pos1=strfind(Note,'ADef    = '); pos2=strfind(Note,'Gain'); pos3=strfind(Note,'dt'); % 'Gain' in new files and 'dt' in older files 
   eval(['aDef=' Note(pos1+9:min(pos2,pos3)-2) ';']);
   if length(aDef)>1; aDef=[]; end
end
...
switch viewMode
   ...
   if isequal(ScanType,'Tilt')||isequal(ScanType,'ADef') set(handles.menuReadout,'Enable','On'); end
%   if ~isempty(Scan); set(handles.editScan,'String',sprintf('%g:%g',Scan([1 end]))); end % Full Scan can be changed to SWin
   ...
   % K-transform fields
   if get(handles.radiobuttonAKx,'Value')==1&&(isequal(ScanType,'Tilt')||isequal(ScanType,'ADef')||(isequal(ScanType,'hv')&&get(handles.radiobuttonHvKz,'value')==1))
...
if (viewMode==1 && (isempty(ScanType) || diff(SWin)==0) ) || (viewMode==4 && (isequal(ScanType,'Tilt')||isequal(ScanType,'ADef')))
...
global FullName ScanType EWin Scan SWin Data Data3 EAlign ERange ACorr AutoAWin Kx AKx AKxWin RangeNorm IDiv scCoeff ISubtr clipNormNeg ...
          X Y Z FracView switchAKx HV thtM tltM aDef presmdX presmdY structMode sharp postsmdX postsmdY clipImgProcNeg alpha
% Note: For processing of angular deflection, LoadData provides global tltM (scalar and empty if scan in Tlt), 
% aDef (scalar and empty if scan in ADef), Scan, and ScanType ('Tilt' or 'ADef')
viewMode=get(handles.menuViewMode,'Value');
% return if no data
...
if switchAKx==0; AKx=ACorr; else surfNx=field2num(handles.editSurfNx); thtMPhys=thtM+surfNx; AKx=Kx; end
...
% Kx-conversion   
   if switchAKx==0
      ...
      if isequal(ScanType,'Tilt')||isequal(ScanType,'ADef'); AKxSliceE=Kxx(HV,mean(EWin),thtM,ASliceE,surfNx,alpha); end 
...
% - ADef scan
      if isequal(ScanType,'ADef')
%         Ky=Kyy(HV,mean(EWin),ASliceE,thtM,Scan(nS1:nPeriod:nS2)+tltM-surfNy,surfNx,alpha);
         ADefM=repmat((Scan(nS1:nPeriod:nS2))',1,size(ASliceE,2));
         Ky=0.5124*sqrt(HV-ePhi+mean(EWin)).*sind(tltM+ADefM-surfNy).*cosd(ASliceE+thtMPhys)+2*pi*HV*sind(alpha+thtMPhys).*sind(tltM)/12400;
         Y=Ky;
      end
% - hv scan  
      if isequal(ScanType,'hv')
         if get(handles.radiobuttonHvKz,'Value')==1
            ...
            Kz=Kzz(HV(nS1:nPeriod:nS2),mean(EWin),thtM,ASliceE,tltM+aDef-surfNy,Vo,surfNx,alpha); Y=Kz;
         end
      end
   end   
%
...
% - ADef scan
      if isequal(ScanType,'ADef') 
         Eb=X; ADefM=repmat(Y,1,size(X,2))-surfNy;
         ThtA=asind((kx+2*pi*HV*cosd(alpha+thtMPhys)/12400)./(0.5124*sqrt(HV-ePhi+Eb)))-thtMPhys;
         Ky=0.5124*sqrt(HV-ePhi+Eb).*sind(tltM+ADefM).*cosd(ThtA+thtMPhys)+2*pi*HV*sind(alpha+thtMPhys).*sind(tltM)/12400;
         Y=Ky;
      end
 % - hv scan
      if isequal(ScanType,'hv')
...
%            Vo=str2num(get(handles.editVo,'String'));  
            ...
            Kxy_2=(HV2-ePhi+Eb).*(sind(ThtA+thtMPhys).^2+(sind(tltM+aDef-surfNy).*cosd(ThtA+thtMPhys)).^2); 
...
%
% *** plot ***
Pos=strfind(FullName,'\'); if isempty(Pos); Pos=strfind(FullName,'/'); end  % parsing universal between Windows and Mac/Linux
...
% - iso-E
      case 4
         ImData(X,Y,ZView,render); axis tight; colorbar; eval(['colormap ''' colorScheme ''''])
% - Iso-E(Angle)         
         if isequal(ScanType,'Tilt')||isequal(ScanType,'ADef')
...
switch viewMode
   ...
   case 4
      ...
      switch(ScanType)
         ...
         case {'Tilt','ADef'}; if switchAKx==0; ylabel([ScanType ' (deg)']); else ylabel('K_{y} (1/A)'); end   
