...
[~, yField, zField, dField] = find_data_fields(dataStr);
...
elseif dataStr.Type == "Eb(kx,ky)"
    for i = 1:length(dataStr.(zField))
        dataStr_frag{i}             = dataStr;
        dataStr_frag{i}.Type     	= "Eb(k)";
        dataStr_frag{i}.index       = dataStr_frag{i}.index(i);
        dataStr_frag{i}.(zField)  	= dataStr_frag{i}.tltM(i);
        dataStr_frag{i}.(dField)    = squeeze(dataStr_frag{i}.(dField)(:,:,i));
    end 
elseif dataStr.Type == "Eb(kx,kz)" || dataStr.Type == "Eb(k,hv)"
    for i = 1:length(dataStr.(zField))
        dataStr_frag{i}             = dataStr;
        dataStr_frag{i}.Type     	= "Eb(k)";
        dataStr_frag{i}.index       = dataStr_frag{i}.index(i);
        ...
        dataStr_frag{i}.(dField)    = squeeze(dataStr_frag{i}.(dField)(:,:,i));
        if size(dataStr_frag{i}.(yField),2) > 1
            dataStr_frag{i}.(yField)    = squeeze(dataStr_frag{i}.(yField)(:,i));
        end
    end 
