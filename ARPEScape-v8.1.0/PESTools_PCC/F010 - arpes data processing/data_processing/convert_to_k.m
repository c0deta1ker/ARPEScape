function dataStr = convert_to_k(dataStr, kconv_args, plot_result)
...
if dataStr.Type == "Eb(k)" || dataStr.Type == "Eb(k,i)" 
    ...
    if length(dataStr.index) == 1
        ...
        dataStr.ky = Kyy(dataStr.hv, dataStr.eb, dataStr.tht, dataStr.thtM, dataStr.tltM + dataStr.tltE, dataStr.surfNormX, incAlpha);
        dataStr.kz = Kzz(dataStr.hv, dataStr.eb, dataStr.thtM, dataStr.tht, dataStr.tltM + dataStr.tltE, v000, dataStr.surfNormX, incAlpha);
    else
        for i = 1:length(dataStr.index)
            ...
            dataStr.ky  = Kyy(dataStr.hv, dataStr.eb(:,:,i), dataStr.tht(:,:,i), dataStr.thtM, dataStr.tltM + dataStr.tltE, dataStr.surfNormX, incAlpha);
            dataStr.kz = Kzz(dataStr.hv, dataStr.eb(:,:,i), dataStr.thtM, dataStr.tht(:,:,i), dataStr.tltM + dataStr.tltE, v000, dataStr.surfNormX, incAlpha);
        end
    end
...
elseif dataStr.Type == "Eb(kx,ky)"
    ...
    if size(dataStr.tltM, 2) > 1
        for i = 1:size(dataStr.tltM, 2)
            ...
            dataStr.ky(:,:,i) = Kyy(dataStr.hv, dataStr.eb(:,:,i), dataStr.tht(:,:,i), dataStr.thtM, dataStr.tltM(i) + dataStr.tltE, dataStr.surfNormX(i), incAlpha);
            dataStr.kz(:,:,i) = Kzz(dataStr.hv, dataStr.eb(:,:,i), dataStr.thtM, dataStr.tht(:,:,i), dataStr.tltM(i) + dataStr.tltE, v000, dataStr.surfNormX(i), incAlpha);
        end
    % Electrostatic Tilt Map
        ...
        for i = 1:size(dataStr.tltE, 2)
            dataStr.surfNormX(i) = SurfNormX(dataStr.hv, eB_ref, kx_ref, dataStr.thtM, thtA_ref(i), incAlpha);
            dataStr.kx(:,:,i) = Kxx(dataStr.hv, dataStr.eb(:,:,i), dataStr.thtM, dataStr.tht(:,:,i), dataStr.surfNormX(i), incAlpha);
            dataStr.ky(:,:,i) = Kyy(dataStr.hv, dataStr.eb(:,:,i), dataStr.tht(:,:,i), dataStr.thtM, dataStr.tltM + dataStr.tltE(i), dataStr.surfNormX(i), incAlpha);
            dataStr.kz(:,:,i) = Kzz(dataStr.hv, dataStr.eb(:,:,i), dataStr.thtM, dataStr.tht(:,:,i), dataStr.tltM + dataStr.tltE(i), v000, dataStr.surfNormX(i), incAlpha);
        end
    end
...
elseif dataStr.Type == "Eb(kx,kz)"
    ...
    for i = 1:size(dataStr.hv, 2)
        ...
        dataStr.ky(:,:,i) = Kyy(dataStr.hv(i), dataStr.eb(:,:,i), dataStr.tht(:,:,i), dataStr.thtM, dataStr.tltM + dataStr.tltE, dataStr.surfNormX(i), incAlpha);
        dataStr.kz(:,:,i) = Kzz(dataStr.hv(i), dataStr.eb(:,:,i), dataStr.thtM, dataStr.tht(:,:,i), dataStr.tltM + dataStr.tltE, v000, dataStr.surfNormX(i), incAlpha);
    end
end
