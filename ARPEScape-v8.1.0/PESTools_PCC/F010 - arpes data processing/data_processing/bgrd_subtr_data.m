...
if strcmpi(normType,"max") || strcmpi(normType,"max-each") || strcmpi(normType,"mean-each")...
        || strcmpi(normType,"none")...
        || strcmpi(normType,"sum-each")
...
for i = 1:size(dataStr.(dField), 3)
    ...
    elseif strcmpi(normType,"sum-each");  norm_data(:,:,i) = norm_data(:,:,i) / sum(sum(norm_data(:,:,i)));
    end
end
% - Forcing maximum value across all scans to be unity if required
if strcmpi(normType,"max") || strcmpi(normType,"max-each") || strcmpi(normType,"sum-each")
...
if plot_result == 1
    ...
    fig.Position(4) = 350;
    subplot(121); hold on;
    ImData(dataStr.(xField)(:,:,1), dataStr.(yField)(:,:,1), dataStr.raw_data(:,:,1));
