...
if solve_type == "lsqcurvefit"
    ...
    [params,rnorm,resid,exitflag,output,lambda,J]  = lsqcurvefit(@(x,roi_xdat) full_pes_function(x,roi_xdat,XPSObj), x0, roi_xdat, roi_ydat, lb, ub, options);  
...
elseif solve_type == "lsqnonlin"
    ...
    options = optimoptions(@lsqnonlin,...
        ...
        'MaxIterations', MaxIter);
    % -- Defining the optimisation options for the hybrid minimisation that takes place after convergence
    [params,rnorm,resid,exitflag,output,lambda,J] = lsqnonlin(@(x) minimize_function(x,XPSObj), x0, lb, ub, options);  
...
% if solve_type == "lsqcurvefit" || solve_type == "lsqnonlin"
%     fitStr.J    = J; 
%     fitStr.ci   = nlparci(params,resid,'jacobian',J);
%     fitStr.ci   = range(fitStr.ci,2) ./ 3.92;   % Standard error
% elseif solve_type == "fmincon" || solve_type == "fminunc"
%     hessian(isnan(hessian)) = 0;
%     fitStr.H    = hessian;
%     fitStr.ci   = sqrt(diag(inv(hessian)));     % Standard error
% if solve_type == "nlinfit"
%     fitStr.CovB = CovB; 
%     fitStr.ci   = sqrt(diag(CovB));
%     fitStr.ci   = nlparci(params,R,"covariance",CovB);
%     fitStr.ci   =  range(fitStr.ci,2) ./3.92;     % Standard error
% else
%     % -- Defining the optimisation options for the simulated annealing method
%     options = statset(...
...
%         'MaxIter', MaxIter,...
%         'TolFun', FuncTol);
%     [P,R,~,CovB,~,~] = nlinfit(roi_xdat, roi_ydat, @(x,roi_xdat) full_pes_function(x,roi_xdat,XPSObj), params, options);
%     fitStr.ci   = nlparci(P,R,"covariance",CovB);
%     fitStr.ci   = range(fitStr.ci,2) ./3.92;     % Standard error
% end
% if isfield(fitStr, 'ci')
%     for i = 1:fitStr.nSTATES
%         % --- Best fit components on new domain
%         fitStr.ciPARAMS(i,:) = fitStr.ci(i:fitStr.nSTATES:end-3);
%         x0 = [iparams{1}(:); ibgrnd{1}(:)]; x0 = x0(i:fitStr.nSTATES:end-3);
%         lb = [iparams{2}(:); ibgrnd{2}(:)]; lb = lb(i:fitStr.nSTATES:end-3);
%         ub = [iparams{3}(:); ibgrnd{3}(:)]; ub = ub(i:fitStr.nSTATES:end-3);
%         val	= lb + (ub-lb);
%         % --- Storing each curve parameter
...
%         M_AREA_UB = PESCurve(fitStr.XX, fitStr.cTYPE(i),...
%                 fitStr.cPARAMS(i,1) + fitStr.ciPARAMS(i,1), fitStr.cPARAMS(i,2) + fitStr.ciPARAMS(i,2), fitStr.cPARAMS(i,3) + fitStr.ciPARAMS(i,3),...
%                 fitStr.cPARAMS(i,4) + fitStr.ciPARAMS(i,4), fitStr.cPARAMS(i,5) + fitStr.ciPARAMS(i,5), fitStr.cPARAMS(i,6) + fitStr.ciPARAMS(i,6),...
%                 fitStr.cPARAMS(i,7) + fitStr.ciPARAMS(i,7), fitStr.cPARAMS(i,8) + fitStr.ciPARAMS(i,8));
%         M_AREA_LB = PESCurve(fitStr.XX, fitStr.cTYPE(i),...
%                 fitStr.cPARAMS(i,1) - fitStr.ciPARAMS(i,1), fitStr.cPARAMS(i,2) - fitStr.ciPARAMS(i,2), fitStr.cPARAMS(i,3) - fitStr.ciPARAMS(i,3),...
%                 fitStr.cPARAMS(i,4) - fitStr.ciPARAMS(i,4), fitStr.cPARAMS(i,5) - fitStr.ciPARAMS(i,5), fitStr.cPARAMS(i,6) - fitStr.ciPARAMS(i,6),...
%                 fitStr.cPARAMS(i,7) - fitStr.ciPARAMS(i,7), fitStr.cPARAMS(i,8) - fitStr.ciPARAMS(i,8));
%         % --- Storing the curve area information
%         fitStr.AREA_ci(i)	= (trapz(fitStr.XX, M_AREA_UB) - trapz(fitStr.XX, M_AREA_LB)) ./3.92;
%     end
% end
end
...
function B = fit_background(x, XPSObj)
    [X, ~, B] = PESBackground(XPSObj.xdat, XPSObj.ydat, XPSObj.fit_args.bTYPE, x(end-2), x(end-1), x(end), XPSObj.fit_args.ibgrnd{4});
    B(isnan(B)) = 0;
...
function M = fit_model(x, XPSObj)
    ...
    for i = 1:XPSObj.fit_args.nSTATES
        ...
        comp_int{i} = PESCurve(XPSObj.roi_xdat, XPSObj.fit_args.cTYPE(i),...
...
function MB = full_pes_function(x, xdat, XPSObj)
    ...
    for i = 1:XPSObj.fit_args.nSTATES
        ...
        pes_args    = x(i:XPSObj.fit_args.nSTATES:end-3);
        % -- Extracting the component intensities
        comp_int{i} = PESCurve(xdat, XPSObj.fit_args.cTYPE(i),...
            pes_args(1), pes_args(2), pes_args(3),...
            pes_args(4), pes_args(5), pes_args(6),...
            pes_args(7), pes_args(8));
    end
