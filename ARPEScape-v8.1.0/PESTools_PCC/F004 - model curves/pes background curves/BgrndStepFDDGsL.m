function [roi_xdat, roi_ydat, roi_bgrnd] = BgrndStepFDDGsL(xdat, ydat, fdd_ef, fdd_T, fdd_fwhm, lhsVal, rhsVal, lhsWin, rhsWin, plot_result)
...
if nargin < 8;          lhsWin = abs(0.01*range(xdat(:))); end
if nargin < 9;          rhsWin = abs(0.01*range(xdat(:))); end
...
fermi_fit = fittype(@(a, b, c, x) FDDGsL(x, fdd_ef, fdd_T, fdd_fwhm, a, b, c));
...
roi_bgrnd = FDDGsL(roi_xdat, fdd_ef, fdd_T, fdd_fwhm, fit1para(1), fit1para(2), fit1para(3));
...
if plot_result == 1
    ...
    gca_props(); title('BgrndStepFDDGsL()'); 
