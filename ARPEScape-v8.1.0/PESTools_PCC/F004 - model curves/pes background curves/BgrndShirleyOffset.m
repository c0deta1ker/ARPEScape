function [roi_xdat, roi_ydat, roi_bgrnd] = BgrndShirleyOffset(xdat, ydat, lambda, delta, lhsVal, rhsVal, lhsWin, rhsWin, plot_result)
...
if nargin < 5;          lhsVal = mean(xdat(:)) - abs(0.25*range(xdat(:))); end
if nargin < 6;          rhsVal = mean(xdat(:)) + abs(0.25*range(xdat(:))); end
if nargin < 7;          lhsWin = abs(0.01*range(xdat(:))); end
if nargin < 8;          rhsWin = abs(0.01*range(xdat(:))); end
...
if isempty(lhsWin);     lhsWin = abs(0.01*range(xdat(:))); end
if isempty(rhsWin);     rhsWin = abs(0.01*range(xdat(:))); end
...
[~, ~, roi_bgrnd_lin]   = BgrndPoly(xdat, ydat, 1, lhsVal, rhsVal, lhsWin, rhsWin);
[~, ~, roi_bgrnd_shrly]	= BgrndShirley(xdat - delta, ydat, lhsVal, rhsVal, lhsWin, rhsWin);
...
if plot_result == 1
    ...
    hMID    = patch([ROI(1), ROI(1), ROI(2), ROI(2), ROI(1)], [-1, 1, 1, -1, -1].*1.5*max(ydat(:)), [0.8 0.9 0.8], 'facealpha', 0.5, 'edgecolor', 'none');
    hLHS    = patch([LHS(1), LHS(1), LHS(2), LHS(2), LHS(1)], [-1, 1, 1, -1, -1].*1.5*max(ydat(:)), [0.6 0.6 0.2], 'facealpha', 0.4, 'edgecolor', 'none');
    hRHS    = patch([RHS(1), RHS(1), RHS(2), RHS(2), RHS(1)], [-1, 1, 1, -1, -1].*1.5*max(ydat(:)), [0.6 0.6 0.2], 'facealpha', 0.4, 'edgecolor', 'none');
