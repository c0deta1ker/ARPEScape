function zdat = ARPESCurve_FDDGsL(xdat, ydat, cTYPE, INT, XLOC, YLOC, XFWHM, YFWHM, MSTAR, FDEF, FDT, FDW, BGR, BIN, BCO, plot_result)
% zdat = ARPESCurve_FDDGsL(xdat, ydat, cTYPE, INT, XLOC, YLOC, XFWHM, YFWHM, MSTAR, FDEF, FDT, FDW, BGR, BIN, BCO, plot_result)
...
ARPES_curve	= ARPESCurve(xdat, ydat, cTYPE, INT, XLOC, YLOC, XFWHM, YFWHM, MSTAR);
% - Multiplying the components together
zdat        = ARPES_curve.* (FDD_curve + BGR.*ydat) + BIN + BCO;
% int         = INT .* (int / max(int));
...
if plot_result == 1
    ...
    img_props(); cbar_props(); title('ARPESCurve_FDDGsL()', 'Interpreter', 'none');
