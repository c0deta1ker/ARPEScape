...
% Last Modified by GUIDE v2.5 11-Aug-2022 19:48:25
...
ePhi=4.5; alpha=9; eB=0; % surfNx calculated for arbitrary eB, and all other calculations assume eB=0
% image the scheme
% - read in image
...
% ---------------- Nominal X-ray incidence ----------------
% --- Executes on selection change in popupmenuAlpha.
function popupmenuAlpha_Callback(hObject,~,handles)
% hObject    handle to popupmenuAlpha (see GCBO)
global alpha
if get(hObject,'Value')==1; alpha=9; else alpha=20; end
% update all outputs if the parameters available
try pushbuttonOrgCalc_Callback(handles.pushbuttonOrgCalc,[],handles); end
try pushbuttonKxCalc_Callback(handles.pushbuttonKxCalc,[],handles); end
try pushbuttonKyCalc_Callback(handles.pushbuttonKyCalc,[],handles); end
try pushbuttonKzCalc_Callback(handles.pushbuttonKzCalc,[],handles); end
...
surfNx = SurfNormX(hv,eB,kx,thtM,thtA,alpha);
% display results
...
switch get(handles.popupmenuKx,'Value')
   case 1
      if isempty(surfNx*hv*thtM*thtA); msgbox('Empty inputs','modal'); return; end
      kx=0.5124*sqrt(hv-ePhi+eB)*sind(thtMPhys+thtA)-2*pi*hv*cosd(alpha+thtMPhys)/12400;
      set(handles.editKxKx,'String',num2str(kx))
   case 2
      if isempty(surfNx*hv*kx*thtA); msgbox('Empty inputs','modal'); return; end 
      ...
      if exitFlag~=1; thtM=[]; msgbox('Solver failure: Check input parameters','modal'); end
      set(handles.editKxThtM,'String',num2str(thtM))
   case 3
      if isempty(surfNx*hv*kx*thtM); msgbox('Empty inputs','modal'); return; end 
      ...
      if exitFlag~=1; thtA=[]; msgbox('Solver failure: Check input parameters','modal'); end
...
global ePhi alpha eB
% read in the parameters
thtA=str2num(get(handles.editKyThtA,'String'));
...
tlt=str2num(get(handles.editKyTlt,'String'));
% calculation of ky/TLT
switch get(handles.popupmenuKy,'Value')
   case 1
      if isempty(thtA*thtM*surfNx*hv*tlt); msgbox('Empty inputs','modal'); return; end 
   ...
   case 2
      if isempty(thtA*thtM*surfNx*hv*ky); msgbox('Empty inputs','modal'); return; end  
      ...
      if exitFlag~=1; tlt=[]; msgbox('Solver failure: Check input parameters','modal'); end      
      set(handles.editKyTlt,'String',num2str(tlt))
end
...
tlt=str2num(get(handles.editKzTlt,'String'));
kx=str2num(get(handles.editKzKx,'String'));
ky=str2num(get(handles.editKzKy,'String'));
if tlt*ky==0&&(tlt+ky)~=0; msgbox('Incompatible tlt and Ky','modal'); return; end  
...
switch get(handles.popupmenuKz,'Value')
   case 1
      if isempty(thtM*surfNx*kx*ky*v000*hv); msgbox('Empty inputs','modal'); return; end  
      % kz=0.5124*sqrt(hv+eB+v000-3.81*(kx^2+ky^2))+2*pi*hv*sind(alpha+thtMPhys)/12400;
      kx_f=kx+2*pi*hv*cosd(alpha+thtMPhys)/12400; ky_f=ky-2*pi*hv*sind(alpha+thtMPhys)*sind(tlt)/12400;
      kz=0.5124*sqrt(hv+eB+v000-3.81*(kx_f^2+ky_f^2))+2*pi*hv*sind(alpha+thtMPhys)/12400;
      if get(handles.popupmenuKzUnits,'Value')==2; kz=kz/tpc; end   % kz units
      set(handles.editKzKz,'String',num2str(kz))
   case 2
      if isempty(thtM*surfNx*kx*ky*v000*kz); msgbox('Empty inputs','modal'); return; end 
      if get(handles.popupmenuKzUnits,'Value')==2; kz=kz*tpc; end   % kz units        
      [hv,~,exitFlag]=fsolve(@(x) 0.5124*sqrt(x+eB+v000-3.81*((kx+2*pi*x*cosd(alpha+thtMPhys)/12400).^2+(ky-2*pi*x*sind(alpha+thtMPhys)*sind(tlt)/12400).^2))+...
         2*pi*x*sind(alpha+thtMPhys)/12400-kz,0,optimset('Display','off','TolFun',1e-12));
      if exitFlag~=1; hv=[]; msgbox('Solver failure: Check input parameters','modal'); end
      hv=real(hv);    % trim the imaginary residual error
      set(handles.editKzHv,'String',num2str(hv))
    case 3
      if isempty(thtM*surfNx*kx*ky*hv*kz); msgbox('Empty inputs','modal'); return; end 
      if get(handles.popupmenuKzUnits,'Value')==2; kz=kz*tpc; end   % kz units 
      v000=((kz-2*pi*hv*sind(alpha+thtMPhys)/12400)/0.5124)^2-hv-eB+3.81*((kx+2*pi*hv*cosd(alpha+thtMPhys)/12400)^2+(ky-2*pi*hv*sind(alpha+thtMPhys)*sind(tlt)/12400).^2);
