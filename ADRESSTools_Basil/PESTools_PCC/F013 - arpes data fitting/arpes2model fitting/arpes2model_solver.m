...
%% 3.5 - LOCAL SOLVER: FIND MINIMUM OF BOUNDED MULTIVARIATE FUNCTION TO MINIMISE STANDARD DEVIATION
elseif solve_type == "fmincon-std2"
    % -- Defining the optimisation options for the simulated annealing method
    options = optimoptions('fmincon',...
        'MaxFunctionEvaluations', MaxFunEvals,...
        'FunctionTolerance', FuncTol,...
...
elseif solve_type == "fminunc-std2"
    % -- Defining the optimisation options for the simulated annealing method
    options = optimoptions('fminunc',...
        'MaxFunctionEvaluations', MaxFunEvals,...
        'FunctionTolerance', FuncTol,...
        ...
        'Display', 'iter-detailed');
    % -- Defining the optimisation options for the hybrid minimisation that takes place after convergence
    [params,fval,exitflag,output,grad,hessian] = fminunc(@(x) minimize_function_std2(x,ARPESObj), x0, options); 
%% - 3.7 - GLOBAL SOLVER: BOUNDED SIMULATED ANNEALING TO MINIMISE STANDARD DEVIATION
elseif solve_type == "simulannealbnd-std2"
    % -- Defining the optimisation options for the hybrid minimisation that takes place after convergence
    hybridopts = optimoptions('fmincon',...
        'MaxFunctionEvaluations', MaxFunEvals,...
        'OptimalityTolerance', OptTol,...
    ...
    % -- Run the simulated annealing method
    % --- (1) Initially optimise all parameters
    [params,fval,exitflag,output] = simulannealbnd(@(x) minimize_function_std2(x,ARPESObj), x0, lb, ub, options);      
end
...
if solve_type == "fmincon-std2" || solve_type == "simulannealbnd-std2" || solve_type == "fminunc-std2"
        fitStr.MINFUN   = minimize_function(params, ARPESObj);
else;   fitStr.MINFUN   = minimize_function_std2(params, ARPESObj);
end
