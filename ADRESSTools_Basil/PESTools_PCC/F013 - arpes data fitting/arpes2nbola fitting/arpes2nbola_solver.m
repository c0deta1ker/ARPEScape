function fitStr = arpes2nbola_solver(arpesStr, cTYPE, iparams, ibgrnd, solve_type)
% fitStr = arpes2nbola_solver(arpesStr, cTYPE, iparams, ibgrnd, solve_type)
...
for i = 1:n
    % --- parameters
    if x0(i) < 0; x0(i) = 0; end        % if INT < 0, then make it 0
    if x0(i+3*n) < 0; x0(i+3*n) = 0; end    % if XFWHM < 0, then make it 0
    if x0(i+4*n) < 0; x0(i+4*n) = 0; end    % if YFWHM < 0, then make it 0
    % --- lower bounds
    if lb(i) < 0; lb(i) = 0; end        % if INT < 0, then make it 0
    if lb(i+3*n) < 0; lb(i+3*n) = 0; end    % if XFWHM < 0, then make it 0
    if lb(i+4*n) < 0; lb(i+4*n) = 0; end    % if YFWHM < 0, then make it 0
    % --- upper bounds
    if ub(i) < 0; ub(i) = 0; end        % if INT < 0, then make it 0
    if ub(i+3*n) < 0; ub(i+3*n) = 0; end    % if XFWHM < 0, then make it 0
    if ub(i+4*n) < 0; ub(i+4*n) = 0; end    % if YFWHM < 0, then make it 0
end
...
[ARPES, MODEL]      = extract_arpes_and_model(params, ARPESObj.arpesStr, cTYPE);
...
fitStr.n        = length(cTYPE);
fitStr.cTYPE  	= cTYPE;
fitStr.INT      = params(1:n);
fitStr.XLOC  	= params(n+1:2*n);
fitStr.YLOC     = params(2*n+1:3*n);
fitStr.XFWHM    = params(3*n+1:4*n);
fitStr.YFWHM    = params(4*n+1:5*n);
fitStr.MSTAR    = params(5*n+1:6*n);
...
fitStr.sbn_kx      = linspace(fitStr.kx_lims(1), fitStr.kx_lims(2), 1e3);
fitStr.sbn_eb      = {};
for i = 1:fitStr.n
    fitStr.sbn_eb{i} = eb_calc(fitStr.sbn_kx, fitStr.XLOC(i), fitStr.YLOC(i), fitStr.MSTAR(i));
end
end

%% DEFINING THE FUNCTION TO BE MINIMISED VIA GLOBAL OPTIMISATION METHODS
function MINFUN = minimize_function(x, ARPESObj)
    [ARPES, MODEL] = extract_arpes_and_model(x, ARPESObj.arpesStr, ARPESObj.fit_args.cTYPE);
