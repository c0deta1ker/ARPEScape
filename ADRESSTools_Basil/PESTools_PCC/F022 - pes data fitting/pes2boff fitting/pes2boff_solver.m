...
% -- Defining the initial conditions and limits of the optimisation method
for i = 1:nSTATES
    indx = (2+i):nSTATES:(length(x0)-6); % +2 to account for MFP and BOFF                    
...
imodelStr      	= mstheory_interp(modelStr, fitStr.BOFF, fitStr.MFP);
...
for i = 1:fitStr.nSTATES
    ...
    fitStr.cPARAMS(i,:) = params((2+i):fitStr.nSTATES:(end-6));
    ...
    fitStr.BE_Z0(i)	= fitStr.DCL(i) - fitStr.BOFF(i);
...
function M = fit_model(x, XPSObj)
    ...
    imodelStr     	= mstheory_interp(XPSObj.modelStr, x(2), x(1));
    ...
    for i = 1:XPSObj.fit_args.nSTATES
        ...
        pes_args    = x((2+i):XPSObj.fit_args.nSTATES:(end-6));
        ...
        comp_int{i} = PESCurve_POT(XPSObj.roi_xdat, XPSObj.fit_args.cTYPE(i),...
            ...
            pes_args(7), pes_args(8), x(1), ZPOT, EPOT);
    end
