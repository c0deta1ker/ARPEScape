function fitStr = arpes2boff1D_solver(arpesStr, modelFunc, cTYPE, iparams, bTYPE, ibgrnd, solve_type)
% fitStr = arpes2boff1D_solver(arpesStr, cTYPE, modelFunc, iparams, bTYPE, ibgrnd, solve_type)
%   Function that is used to fit N-parabolic dispersions to an EDC cut
%   through ARPES data based on a set of model functions (modelFunc) that
...
[roi_xdat, roi_ydat, roi_bgrnd] = PESBackground(arpesStr.xdat, arpesStr.ydat, bTYPE, x0(end-2), x0(end-1), x0(end), ibgrnd{4});
...
%% 4.4 - Storing the final fit parameters
...
function [X, D] = fit_data(x, ARPESObj)
    [X, D, ~] = PESBackground(ARPESObj.xdat, ARPESObj.ydat, ARPESObj.fit_args.bTYPE, x(end-2), x(end-1), x(end), ARPESObj.fit_args.ibgrnd{4});
    D(isnan(D)) = 0;
...
function B = fit_background(x, ARPESObj)
    [~, ~, B] = PESBackground(ARPESObj.xdat, ARPESObj.ydat, ARPESObj.fit_args.bTYPE, x(end-2), x(end-1), x(end), ARPESObj.fit_args.ibgrnd{4});
    B(isnan(B)) = 0;
...
function M = fit_model(x, ARPESObj)
    % -- Extracting
    modelFunc  	= ARPESObj.fit_args.modelFunc;
...
function MB = full_pes_function(x, xdat, ARPESObj)
    ...
    [B_xdat, ~, B] = PESBackground(xdat, ARPESObj.roi_ydat, ARPESObj.fit_args.bTYPE, x(end-2), x(end-1), x(end), ARPESObj.fit_args.ibgrnd{4});
